...
export class CursorProvider implements AIProvider {
  ...
  private buildFullPrompt(prompt: string, context: TaskContext): string {
    ...
    lines.push(prompt);
    lines.push('');
    lines.push('## CRITICAL: Response Format (STRICT)');
    lines.push('');
    lines.push('**RETURN PURE JSON ONLY. NO MARKDOWN CODE BLOCKS.**');
    lines.push('');
    lines.push('### REQUIRED:');
    lines.push('- First character of your response MUST be: {');
    lines.push('- Last character of your response MUST be: }');
    lines.push('- Valid JSON matching the schema below');
    lines.push('');
    lines.push('### FORBIDDEN:');
    lines.push('- Markdown code blocks (```json ... ```) - causes parsing failures');
    lines.push('- Narrative text before or after the JSON');
    lines.push('- Escaped JSON strings or nested JSON wrappers');
    lines.push('');
    ...
    lines.push('');
    lines.push('## EXAMPLE RESPONSE (notice: NO markdown wrapper):');
    lines.push('');
  ...
  private buildStrictJsonPrompt(context: TaskContext): string {
    ...
    lines.push('}');
    lines.push('');
    lines.push('GENERATE THE CODE NOW. START WITH { CHARACTER.');
